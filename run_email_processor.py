...
def split_text_with_llm(text_content: str) -> (dict | None, list):
    ...
    classification_prompt = f"""
        ...
        # 判断の基本原則
        1.  **最新の返信部分が中心的な判断材料です。
        2.  最新の返信が、引用部分の情報を補足して新しい提案**を行っている場合（例：技術者情報に対して案件情報を送る）、それは登録対象です。
        3.  最新の返信が、引用部分の情報に対する単なる確認、感謝、承諾、質問である場合（例：「ありがとうございます」「進めてください」「承知しました」）、それは登録対象外（OTHER）です。
        ...
        ### 判断:
        最新の返信部分で、引用された技術者情報（H.K様）に対して具体的な新しい案件情報を提示している。これは「PROJECT_INFO」として登録すべき。
        ---
        ## 例2: 登録すべきでないケース（OTHER）
        ### メール内容:
        > ■案件名：某鉄道予約システム機能増強
        > ■必須スキル：Java開発経験, C言語開発経験...
