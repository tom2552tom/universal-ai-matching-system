...
def split_text_with_llm(text_content: str) -> (dict | None, list):
    ...
    classification_prompt = f"""
        あなたは、IT業界の営業担当者間のメールを分析する専門家です。
        あなたのタスクは、メールスレッド全体を読み解き、そのメールが「新たにDBに登録すべき情報」を含んでいるかを判断することです。

        # 判断の基本原則
        1.  **最新の返信部分が中心的な判断材料**です。
        2.  最新の返信が、引用部分の情報を補足して**新しい提案**を行っている場合（例：技術者情報に対して案件情報を送る、案件情報に対して技術者情報を送る）、それは登録対象です。
        3.  最新の返信が、引用部分の情報に対する**単なる確認、感謝、承諾、質問**である場合（例：「ありがとうございます」「進めてください」「承知しました」）、それは登録対象外（OTHER）です。

        # 具体例による学習（Few-shot Learning）

        ---
        ## 例1: 登録すべきケース（PROJECT_INFO）
        ### メール内容:
        > ビジネスパートナーさま 各位
        > ...
        > ＜技術者＞ 氏名：H.K（34歳 男性）スキル：C言語、Java...
        
        ユニバーサルコンツェルン 北島 様
        いつもお世話になっております。オプティム武田です。
        下記、貴社H.K様にマッチすると考えご提案致しました。
        ■案件名：某鉄道予約システム機能増強
        ■必須スキル：Java開発経験, C言語開発経験...
        
        ### 判断:
        最新の返信部分で、引用された技術者情報に対して具体的な新しい案件情報を提示している。これは「PROJECT_INFO」として登録すべき。
        ---
        ## 例2: 登録すべきケース（ENGINEER_INFO）  【← この例を追加】
        ### メール内容:
        > パートナー各位
        > いつも大変お世話になっております。
        > ■案件名：大手ECサイト保守開発
        > ■必須スキル：PHP, Laravel, Vue.js...
        
        株式会社ABC パートナー営業部 御中
        お世話になっております。株式会社XYZの佐藤です。
        貴社募集のECサイト保守開発の件、弊社所属の下記エンジニアをご提案いたします。
        PHP, Laravelの経験が豊富で、即戦力として貢献可能です。
        ---
        氏名：鈴木 一郎（38歳 男性）
        稼働：即日～
        スキル：PHP, Laravel, JavaScript, Vue.js, AWS...
        
        ### 判断:
        最新の返信部分で、引用された案件情報に対して具体的な新しい技術者情報を提示している。これは「ENGINEER_INFO」として登録すべき。
        ---
        ## 例3: 登録すべきでないケース（OTHER）
        ### メール内容:
        > ■案件名：某鉄道予約システム機能増強
        > ■必須スキル：Java開発経験, C言語開発経験...
        
        オプティム 武田様
        いつもお世話になっております。ユニバーサルコンツェルンの北島です。
        この度は案件をご紹介頂きありがとうございます。是非、進めて頂けますと幸いです。
        
        ### 判断:
        最新の返信部分は、引用された案件情報に対する感謝と承諾のみ。新しい具体的な情報を提供していないため、これは「OTHER」として扱う。
        ---
        ## 例4: 登録すべきでないケース（SCHEDULING）
        ### メール内容:
        > ■案件名：某鉄道予約システム機能増強
        > 技術者：山田 太郎様
        
        山田様の件、顧客と面談を調整しております。
        つきましては、下記候補日時よりご都合のよろしい時間帯をお知らせいただけますでしょうか。
        ・11月13日(木) 14:00-15:00
        ・11月14日(金) 16:00-17:00
        
        ### 判断:
        最新の返信部分は、面談の日程調整が主目的。これは「SCHEDULING」として扱う。
        ---

        # カテゴリ定義
        - `PROJECT_INFO`: メール全体として、DBに登録すべき新しい案件情報を提供している。
        - `ENGINEER_INFO`: メール全体として、DBに登録すべき新しい技術者情報を提供している。
        - `SCHEDULING`: 最新の返信が、面談や会議の日程調整を主目的としている。
        - `BILLING`: 最新の返信が、請求や支払いに関する連絡を主目的としている。
        - `OTHER`: 上記に当てはまらない、単なる確認、感謝、挨拶、進捗報告などの業務連絡。

        # 本番: 以下のメールを分析し、最も適切なカテゴリを一つだけ回答してください。
