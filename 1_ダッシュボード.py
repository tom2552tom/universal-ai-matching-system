...
if not results:
    ...
    if not results_to_display:
        ...
        st.divider()
        if total_pages > 1:
            def set_page(page_num): st.session_state.current_page = page_num
            PAGES_TO_SHOW = 5
            pages = []
            current_page = st.session_state.current_page
            if total_pages <= PAGES_TO_SHOW: pages = list(range(1, total_pages + 1))
            else:
                pages.append(1)
                half_window = (PAGES_TO_SHOW - 2) // 2
                start = max(2, current_page - half_window)
                end = min(total_pages - 1, current_page + half_window)
                if start > 2: pages.append("...")
                for i in range(start, end + 1): pages.append(i)
                if end < total_pages - 1: pages.append("...")
                pages.append(total_pages)

            pagination_container = st.container()
            with pagination_container:
                st.markdown("""<style>.pagination-container { display: flex; justify-content: center; align-items: center; gap: 5px; } .pagination-container .stButton button { width: 40px; height: 40px; } .current-page { font-weight: bold; background-color: #ff4b4b; color: white; border-radius: 5px; padding: 8px 14px; }</style>""", unsafe_allow_html=True)
                st.markdown('<div class="pagination-container">', unsafe_allow_html=True)
                if st.button("«", key="pg_prev", disabled=current_page <= 1): set_page(current_page - 1); st.rerun()
                for page in pages:
                    if page == "...": st.markdown("<span>...</span>", unsafe_allow_html=True)
                    elif page == current_page: st.markdown(f'<span class="current-page">{page}</span>', unsafe_allow_html=True)
                    else:
                        if st.button(str(page), key=f"pg_{page}"): set_page(page); st.rerun()
                if st.button("»", key="pg_next", disabled=current_page >= total_pages): set_page(current_page + 1); st.rerun()
