...
with st.container(border=True):
    # a. アイコンとタイトルを横並びにする
    col_icon, col_title_text = st.columns([1, 15], vertical_alignment="center")
    with col_icon:
    ...
    # ▼▼▼【ここからが修正の核】▼▼▼
    try:
        # --- 1. キャッシュキーを生成するためのデータを事前に取得 ---
        
        # a) 現在時刻を「5分単位」で丸める
        now = datetime.now()
        minutes_rounded = (now.minute // 5) * 5
        time_block = now.replace(minute=minutes_rounded, second=0, microsecond=0).isoformat()

        # b) 最新のフィードバックIDを取得する
        latest_feedback_log = be.get_feedback_and_learning_logs(limit=1)
        latest_feedback_id = latest_feedback_log[0]['result_id'] if latest_feedback_log else 0

        # --- 2. 安定しつつも、変化を捉えられるキャッシュキーを作成 ---
        stable_cache_key = f"{time_block}_{latest_feedback_id}"

        # --- 3. キャッシュされる関数を定義 ---
        #    この関数は、キャッシュキーが変更された場合にのみ実行される
        @st.cache_data(ttl=60)
        def generate_dynamic_ai_advice(cache_key: str):
            # 関数内部で、AIが必要とする最新のデータをすべて取得する
            latest_dashboard_data = be.get_live_dashboard_data()
            latest_feedback = be.get_feedback_and_learning_logs(limit=1)
            latest_news = be.get_latest_news_from_feeds()
            ...
            context_summary = {
                "直近の活動": {
                    "新規案件登録数": latest_dashboard_data.get('jobs_today', 0),
                    "採用決定数": latest_dashboard_data.get('adopted_count_today', 0),
                },
                "現在のパイプライン": {
                    "提案準備中": latest_dashboard_data.get('funnel_data', {}).get('提案準備中', 0),
                },
                "最新の学習トピック": latest_learning_topic,
                "世の中の動き": latest_news
            }
            ...
            # --- プロンプト生成 ---
            prompt = f"""
            あなたは、IT人材紹介事業を支援する、知的で好奇心旺盛なAIパートナーです。
            あなたは、的確な業務アドバイスだけでなく、世の中の様々な最新ニュースにも精通しており、チームに新しい視点を提供します。
            ...
            # あなたが生成するメッセージのカテゴリー:
            ### 1. 【アクティブ状態への対応】 (優先度低)
            - (もし、採用決定が1件以上だったら) -> 「採用決定、素晴らしい成果です。この成功パターンを分析し、類似案件への横展開を検討しましょう。」
            - (もし、直近の新規登録が多いだったら) -> 「多くの新着情報が届いています。情報が新鮮なうちにAI再評価を行い、最良のマッチングを発見しましょう。」
            ### 2. 【アイドル状態への対応】(優先度低)
            - (もし、提案準備中の案件が滞留していたら) -> 「提案準備中の案件がいくつか停滞しています。この静かな時間を使って、提案内容を練り直してはいかがでしょうか。」
            ### 3. 【自己学習の共有】(優先度低)
            - (もし、最新の学習トピックが「単価の妥当性の場合) -> 「💡 先日のフィードバックから、単価の許容範囲について新たな知見を得ました。今後のマッチング精度向上にご期待ください。」
            ### 4. ニュース(最優先)
            - (指示): 与えられた「世の中の動き」の中から、IT業界、政治経済、投資、AIまたはビジネスに関連しそうだと感じたトピックを一つだけ選び、それについてコメントしてください。
            ### 5.  【世間話・コラム】(優先度低)
            - (話題の例: 健康) -> 「最近の分析によると、短時間の集中とこまめな休憩が生産性を最も高めるそうです。一度、席を立ってストレッチなどいかがでしょうか。」

            # 制約:
            - 常に知的で、丁寧、かつプロフェッショナルなトーンを維持してください。
            - 出力は生成したメッセージの文章のみです。
            - これは絶対的な制約です。必ず守ってください。【アクティブ状態への対応】などタイトルは絶対に含まないでください。
            ...
            try:
                advice = be.generate_text(prompt, max_tokens=300)
                if advice and advice.strip():
                    return advice.strip().replace('"', '')
                return "現在、新しいアドバイスを準備中です。少々お待ちください。"
            except Exception as e_inner:
                print(f"AIアドバイスの生成に失敗しました: {e_inner}")
                return "システムは正常に稼働していますが、現在アドバイスを生成できません。"

        # --- 4. 安定したキャッシュキーを使って関数を呼び出し、結果を表示 ---
        advice = generate_dynamic_ai_advice(stable_cache_key)
        ...
        with advice_placeholder.container():
            st.error("AIアドバイスの表示準備中にエラーが発生しました。")
