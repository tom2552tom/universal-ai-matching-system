...
def re_evaluate_and_match_single_job(job_id):
    """
    指定された単一の案件IDに対して、AIによる再評価と再マッチングを実行する。
    """
    conn = get_db_connection()
    try:
        # 1. 対象案件の最新情報を取得
        job_record = conn.execute("SELECT * FROM jobs WHERE id = ?", (job_id,)).fetchone()
        if not job_record:
            st.error(f"案件ID:{job_id} が見つかりませんでした。")
            return False
        ...
        # 2. LLMでドキュメントを再生成
        parsed_data = split_text_with_llm(full_text_for_llm, 'job') # item_typeを'job'に指定
        if not parsed_data or not parsed_data.get("jobs"):
            st.error("LLMによる再評価で、案件情報の抽出に失敗しました。")
            return False
        ...
        # 3. データベースのドキュメントとプロジェクト名を更新
        cursor = conn.cursor()
        new_project_name = item_data.get('project_name', job_record['project_name'])
        cursor.execute("UPDATE jobs SET document = ?, project_name = ? WHERE id = ?", (new_full_document, new_project_name, job_id))
        
        # 4. 既存の関連マッチング結果を一旦削除
        cursor.execute("DELETE FROM matching_results WHERE job_id = ?", (job_id,))
        st.write(f"案件ID:{job_id} の既存マッチング結果をクリアしました。")
        ...
        run_matching_for_item(job_data_for_matching, 'job', cursor, now_str)
